...
// ── HUMANO ──
  ...
  socket.on('submit_human_answer', async ({answer, instructions}) => {
    ...
    // Si hay supervivientes de una pregunta anterior, reutilizarlos
    const survivors = (room._survivors||[]).filter(s=>!s.isHuman);
    const survivorCount = survivors.length;
    // Solo generar bots para los slots vacíos (10 - supervivientes)
    const botsNeeded = Math.max(0, 10 - survivorCount);
    const bots = botsNeeded > 0
      ? await genBotAnswers(room.question,answer,room.profile,room.extraContext,botsNeeded,writingSamples,bannedWords)
      : [];
    room._survivors = null; // limpiar después de usar

    // Actualizar respuesta de supervivientes para nueva pregunta
    const survivorEntries = survivors.map((s,i)=>({
      ...s,
      answer: bots[i] || s.answer, // reemplazar respuesta con la nueva generada para ese slot
      eliminated: false
    }));

    // Pool: humano + supervivientes actualizados + bots nuevos para slots vacíos
    const newBots = bots.slice(survivorCount).map((a,i)=>({
      id:`bot_new_${i}`,answer:a,isHuman:false,eliminated:false
    }));
  ...
  socket.on('request_new_question', () => {
    ...
    // Guardar los supervivientes actuales (bots que aún no han sido eliminados)
    // Para que al responder la nueva pregunta solo participen esos + el humano
    const survivors = (room.activeAnswers||[]).filter(a=>!a.eliminated);
    room._survivors = survivors; // se usan en submit_human_answer
    ...
    io.to(room.interrogatorId).emit('your_turn_to_ask',{
      penalty,newQuestionCount:room.newQuestionCount,
      currentRound:room.currentRound,totalRounds:tr,
      survivorsCount:survivors.filter(s=>!s.isHuman).length // cuántos bots quedan
    });
    io.to(room.humanId).emit('waiting_for_question',{
      currentRound:room.currentRound,totalRounds:tr,
      survivorsCount:survivors.filter(s=>!s.isHuman).length
    });
  ...
  socket.on('submit_elimination', ({labels,finalGuess}) => {
    ...
    // 100 pts por cada bot eliminado correctamente
    // 'botsEliminated' = cuántos bots quitó antes de terminar la ronda
    const botsCorrectlyEliminated = ()=>
      room.allAnswers.filter(a=>!a.isHuman&&a.eliminated).length;
    const interPts = () => botsCorrectlyEliminated() * 100;
    const humanSurvivePts = () => botsCorrectlyEliminated() * 50; // humano gana mitad de lo que eliminó
    ...
    if(finalGuess){
      ...
      finishRound(isH?interPts():-100,isH?0:humanSurvivePts(),
    ...
    if(elimH){
      const hE=room.allAnswers.find(a=>a.isHuman);
      finishRound(-humanSurvivePts(),humanSurvivePts(),
