...
async function initDB() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS banned_words (
      ...
      room_player_key VARCHAR(200) NOT NULL,
      word VARCHAR(100) NOT NULL,
      times_caught INTEGER DEFAULT 1,
      added_at TIMESTAMP DEFAULT NOW(),
      UNIQUE(room_player_key, word)
    );
    CREATE INDEX IF NOT EXISTS idx_banned ON banned_words(room_player_key);
  `);
...
// BANNED WORDS — palabras que la IA no puede usar con este grupo de jugadores
async function getBannedWords(roomKey){
  const r=await pool.query('SELECT word,times_caught FROM banned_words WHERE room_player_key=$1 ORDER BY times_caught DESC',[roomKey]);
  return r.rows;
}
async function addBannedWord(roomKey,word){
  const w=word.toLowerCase().trim().replace(/[^a-záéíóúüñ\w]/gi,'');
  if(!w||w.length<2) return;
  await pool.query('INSERT INTO banned_words(room_player_key,word)VALUES($1,$2) ON CONFLICT(room_player_key,word) DO UPDATE SET times_caught=banned_words.times_caught+1,added_at=NOW()',[roomKey,w]);
}
async function removeBannedWord(roomKey,word){
  await pool.query('DELETE FROM banned_words WHERE room_player_key=$1 AND word=$2',[roomKey,word.toLowerCase().trim()]);
}
async function detectAndLearnBannedWords(roomKey,botAnswers,humanAnswer){
  // Detecta palabras que aparecen en MUCHAS respuestas bot pero NO en la humana
  const humanTokens=new Set(humanAnswer.toLowerCase().split(/\s+/).map(w=>w.replace(/[^a-záéíóúüñ\w]/gi,'')).filter(w=>w.length>3));
  const freq={};
  botAnswers.forEach(a=>{
    const tokens=[...new Set(a.toLowerCase().split(/\s+/).map(w=>w.replace(/[^a-záéíóúüñ\w]/gi,'')).filter(w=>w.length>3))];
    tokens.forEach(w=>{ freq[w]=(freq[w]||0)+1; });
  });
  const threshold=Math.ceil(botAnswers.length*0.55);
  const suspicious=Object.entries(freq).filter(([w,c])=>c>=threshold&&!humanTokens.has(w)).map(([w])=>w);
  for(const w of suspicious) await addBannedWord(roomKey,w);
  return suspicious;
}

module.exports={initDB,createUser,getUserByUsername,getUserById,saveWritingSample,getWritingSamples,getHumanoLeaderboard,saveSvScore,getSvLeaderboard,saveDndAdventure,getDndAdventures,getDndAdventureById,incrementDndPlayCount,saveDndSession,getUniverses,getUniverseById,createUniverse,updateUniverseLore,addUniverseEvent,getUniverseHistory,getUniverseLegends,createCharacter,getCharactersByUser,getCharacterById,updateCharacter,killCharacter,getMissionsForUniverse,saveMission,getMissionById,incrementMissionPlayCount,startCharacterAdventure,updateCharacterAdventure,getCharacterAdventures,saveFullSession,getOngoingAdventure,getShopItems,getBannedWords,addBannedWord,removeBannedWord,detectAndLearnBannedWords};
