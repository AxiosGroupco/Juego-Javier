...
io.on('connection', (socket) => {
  ...
  socket.on('dnd_enter_universe', async ({characterId, universeId}) => {
    ...
    if(!authUser) return socket.emit('dnd_error','Debes iniciar sesión');
    try {
      const [character, universe] = await Promise.all([
      ...
      // Obtener historia, leyendas y misiones en paralelo
      const [history, legends] = await Promise.all([
        db.getUniverseHistory(universeId, 30),
        db.getUniverseLegends(universeId)
      ]);

      socket.emit('dnd_processing', {message:`Entrando a ${universe.name}...`});

      // ── OPTIMIZACIÓN: intro + misiones en UNA sola llamada ───────
      const {introNarration, missions: rawMissions} = await dnd.generateUniverseEntry(universe, charData, history, legends);

      // Guardar misiones en DB y devolver con IDs
      // Primero verificar si ya hay misiones disponibles en DB para este universo/nivel
      const existingMissions = await db.getMissionsForUniverse(universeId, 1, (character.level||1)+5);
      let savedMissions = [];
      if(existingMissions.length >= 3) {
        // Reusar misiones existentes si hay suficientes (ahorra una llamada a Claude)
        savedMissions = existingMissions.slice(0,4).map(m=>({
        ...
        // Generar nuevas misiones solo si no hay suficientes
      ...
      socket.emit('dnd_universe_entered', {
        character: charData,
        universe,
        introNarration: introNarration||universe.lore?.openingNarration||'Bienvenido al universo.',
    ...
    if(allActed) {
      ...
      try {
        ...
        if(session.dbAdventureId) {
          try { await db.updateCharacterAdventure(session.dbAdventureId, {history:session.history.slice(-10)}, 'ongoing', null, session.chapter); } catch(e) {}
        }
