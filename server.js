...
// ── API SURVIVAL LEADERBOARD ───────────────────────────────────
app.get('/api/sv-leaderboard', async (req,res) => {
  try {
    const [solo, duo] = await Promise.all([
      db.getSvLeaderboard('solo', 20),
      db.getSvLeaderboard('duo', 20)
    ]);
    res.json({solo, duo});
  } catch(e) {
    console.error('sv-leaderboard error:', e.message);
    res.json({solo:[], duo:[]});
  }
});
...
io.on('connection', (socket) => {
  ...
  socket.on('create_room', ({name, game, solo}) => {
    ...
    if(solo){
      ...
      if(game==='humano'){rooms[code].interrogatorId='claude';startHumanoSolo(code,rooms[code]);}
        ...
        // D&D solo: emitir room_created para que el cliente dispare dnd_start
  ...
  // Unirse a sala D&D con personaje creado
  socket.on('dnd_join', ({code, character}) => {
    const room = rooms[code];
    if(!room || room.game !== 'dnd') return socket.emit('error','Sala D&D no encontrada');
    const existing = room.players.find(p=>p.id===socket.id);
    if(!existing) {
      room.players.push({id:socket.id, name:character.name, role:'player', score:0});
    }
    // Guardar personaje en sesión dnd
    const session = dnd.getDndSession(code);
    if(session) {
      const sp = session.players.find(p=>p.id===socket.id);
      if(sp) { Object.assign(sp, character); }
      else { session.players.push({id:socket.id, ...character, hp:20,maxHp:20,str:10,dex:10,int:10,cha:10,items:[],alive:true}); }
    }
    socket.join(code); socket.roomCode=code;
    io.to(code).emit('dnd_party_update', {players: room.players});
  });

  // Iniciar aventura D&D
  socket.on('dnd_start', async ({code, adventureId, characters}) => {
    const room = rooms[code];
    if(!room) return;
    // Obtener o crear aventura
    let adventure;
    try {
      if(adventureId) {
        const row = await db.getDndAdventureById(adventureId);
        adventure = row ? {...row.adventure_json, id:row.id} : null;
        if(adventure) await db.incrementDndPlayCount(adventureId);
      }
      if(!adventure) {
        const theme = dnd.ADVENTURE_THEMES[Math.floor(Math.random()*dnd.ADVENTURE_THEMES.length)];
        adventure = await dnd.generateAdventure(theme, room.players.length || 1);
        if(adventure) {
          adventure.id = await db.saveDndAdventure(adventure.title, adventure.summary, theme, adventure);
        }
      }
    } catch(e) { console.error('DnD start error:', e.message); }

    if(!adventure) {
      return io.to(code).emit('dnd_error', '❌ No se pudo cargar la aventura. Intenta de nuevo.');
    }

    // Construir lista de personajes con socket.id correcto
    let chars;
    if(characters && characters.length > 0) {
      // Asignar socket.id del emisor al primer personaje (modo solo)
      chars = characters.map((c, i) => ({
        ...c,
        id: room.players[i]?.id || socket.id
      }));
    } else {
      // Fallback: usar jugadores del room
      chars = room.players.map(p => ({
        id: p.id,
    ...
    const session = dnd.createDndSession(code, adventure, chars);
    room.dndAdventureId = adventure.id;

    // Emitir intro con imagen y música
    const musicParams = dnd.getMusicParams('mysterious', 'exploration');
    const svg = await dnd.generateSceneSVG(
      `${adventure.setting}, epic fantasy opening scene`,
      adventure.atmosphere === 'dark' ? 'horror' : adventure.atmosphere || 'mysterious',
      'exploration'
    );

    io.to(code).emit('dnd_started', {
      adventure,
      session: serializeSession(session),
      svg,
      musicParams,
      narration: adventure.openingScene,
      choices: [
        {id:'A', text:'Explorar los alrededores con cautela', risk:'low'},
        {id:'B', text:'Adentrarse directamente en la mazmorra', risk:'high'},
        {id:'C', text:'Preparar el campamento y planificar', risk:'low'}
      ],
      phase: 'decision'
    });
  ...
  socket.on('dnd_action', async ({code, choiceId, customAction}) => {
    ...
    // Registrar acción de este jugador
    session.pendingActions[socket.id] = customAction || choiceId;

    // Notificar al grupo quién ya actuó
    io.to(code).emit('dnd_action_received', {
      playerId: socket.id,
      playerName: player.characterName || player.name,
      choiceId: customAction ? `"${customAction}"` : choiceId,
      pendingCount: Object.keys(session.pendingActions).length,
      totalPlayers: session.players.filter(p=>p.alive).length
    });

    // Cuando todos actúan (o grupo de 1), procesar el turno
    ...
    if(allActed) {
      ...
      // Construir acción grupal
      const actionsDesc = alivePlayers.map(p=>{
        const act = session.pendingActions[p.id];
        return `${p.characterName||p.name} eligió: ${act}`;
      }).join(' | ');
      ...
      try {
        // Generar escena
        const sceneResult = await dnd.generateScene(session, actionsDesc, lead);
        ...
        if(sceneResult.mechanicalEffects) {
          for(const fx of sceneResult.mechanicalEffects) {
            ...
            if(fx.stat==='hp') {
              ...
              if(target.hp===0) { target.alive=false; effectsLog.push(`💀 ${target.characterName} cayó en combate!`); }
              else effectsLog.push(`${fx.change>0?'❤️':'💔'} ${target.characterName}: ${fx.change>0?'+':''}${fx.change} HP → ${target.hp}HP`);
        ...
        // Actualizar ubicación y historial
        if(sceneResult.currentLocation) session.currentLocation = sceneResult.currentLocation;
        session.history.push({
          player: lead.characterName||lead.name,
          action: actionsDesc,
          outcome: sceneResult.narration?.substring(0,100)+'...'
        });
        ...
        if(sceneResult.xpGrants && Array.isArray(sceneResult.xpGrants)) {
          for(const grant of sceneResult.xpGrants) {
            if(grant.toAll) {
              dnd.grantXPAll(session, grant.amount||10, grant.reason||'acción', io, code);
            } else if(grant.playerId) {
              dnd.grantXP(session, grant.playerId, grant.amount||10, grant.reason||'acción', io, code);
            }
          }
        }
        // XP automático por completar capítulo
        ...
        // XP automático por jefe
        if(sceneResult.sceneType==='boss' && sceneResult.mechanicalEffects?.some(fx=>fx.stat==='hp'&&fx.change<-10)) {
          dnd.grantXPAll(session, dnd.XP_REWARDS.boss_killed, 'jefe derrotado', io, code);
        }

        // Generar imagen SVG en paralelo
        const svg = await dnd.generateSceneSVG(
          sceneResult.imagePrompt || `${session.currentLocation} fantasy scene`,
          sceneResult.mood || 'mysterious',
          sceneResult.sceneType || 'exploration'
        );

        const musicParams = dnd.getMusicParams(sceneResult.mood||'mysterious', sceneResult.sceneType||'exploration');

        // Game over check
        if(sceneResult.gameOver || session.players.every(p=>!p.alive)) {
          session.phase = 'gameover';
          try { await db.saveDndSession(session.adventure.id, session.players.map(p=>p.characterName||p.name).join(', '), 'defeat', session.chapter); } catch {}
          io.to(code).emit('dnd_gameover', {
            narration: sceneResult.narration,
            reason: sceneResult.gameOverReason || 'El grupo cayó en la oscuridad...',
            session: serializeSession(session),
            svg, musicParams: dnd.getMusicParams('horror','combat')
          });
          return;
        }
        ...
        if(sceneResult.chapterComplete && session.chapter >= (session.adventure.estimatedChapters||5)) {
          ...
          try { await db.saveDndSession(session.adventure.id, session.players.map(p=>p.characterName||p.name).join(', '), 'victory', session.chapter); } catch {}
          io.to(code).emit('dnd_victory', {
            narration: sceneResult.narration,
            session: serializeSession(session),
            svg, musicParams: dnd.getMusicParams('triumphant','rest')
          });
          return;
        }
  ...
  // Gastar puntos de habilidad/stat durante la partida
  socket.on('dnd_spend_points', ({code, skillId, statKey}) => {
    ...
    if(skillId) {
      // Gastar punto de habilidad
      if(!(player.skillPtsAvailable>0)) return socket.emit('dnd_error','Sin puntos de habilidad disponibles');
      ...
      if(!reqMet) return socket.emit('dnd_error','Necesitas aprender las habilidades requeridas primero');
      if((player.learnedSkills||[]).includes(skillId)) return socket.emit('dnd_error','Ya tienes esta habilidad');
      // Verificar límite de 5 estrellas
      const has5 = (player.learnedSkills||[]).some(id=>skills.SKILL_INDEX[id]?.stars===5);
      if(skill.stars===5 && has5) return socket.emit('dnd_error','Solo puedes tener 1 habilidad de 5⭐');
      // Calcular costo (x2 si es de otra clase)
      const ownClass = skills.ALL_SKILLS[player.characterClass]||[];
      const isOwn = ownClass.some(s=>s.id===skillId) || skill.cat==='Universal';
      const cost = isOwn ? 1 : 2; // en puntos de habilidad (cada punto = 1 habilidad propia o 0.5 externa)
      if(player.skillPtsAvailable < cost) return socket.emit('dnd_error',`Necesitas ${cost} puntos de habilidad`);
      ...
      session.log.push(`⬆️ ${player.characterName} aprendió: ${skill.name} (${'⭐'.repeat(skill.stars)})`);
      socket.emit('dnd_points_spent', {type:'skill', skillId, remaining: player.skillPtsAvailable, statRemaining: player.statPtsAvailable});
      io.to(code).emit('dnd_scene_update', {session: serializeSession(session), log: session.log.slice(-5)});
    }

    if(statKey && ['str','dex','int','cha','hp'].includes(statKey)) {
      if(!(player.statPtsAvailable>0)) return socket.emit('dnd_error','Sin puntos de stat disponibles');
      player[statKey] = (player[statKey]||10) + 1;
      if(statKey==='hp') { player.maxHp++; player.hp++; }
      player.statPtsAvailable--;
      player.pendingLevelUp = player.skillPtsAvailable>0 || player.statPtsAvailable>0;
      session.log = session.log||[];
      session.log.push(`📊 ${player.characterName} mejoró ${statKey.toUpperCase()} → ${player[statKey]}`);
  ...
  // Solicitar imagen de un personaje D&D
  socket.on('dnd_char_portrait', async ({code, character}) => {
    // No generamos imagen real, usamos el sistema SVG del cliente (canvas 2D)
    socket.emit('dnd_char_portrait_ok', {character});
  });
...
function serializeSession(session) {
  return {
    players: session.players.map(p=>({
      ...p,
      xpToNext: dnd.getXpToNextLevel(p.level||1)
    })),
    chapter: session.chapter,
    currentLocation: session.currentLocation,
    phase: session.phase,
    adventureTitle: session.adventure?.title,
    estimatedChapters: session.adventure?.estimatedChapters || 5
  };
}
