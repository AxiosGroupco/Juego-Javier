...
function torretasTurn(sv, code) {
  ...
  sv.torretas.forEach(t=>{
    ...
    // Buscar mob en línea recta (atraviesa barreras, no obstáculos de pos)
    // Buscar el mob más cercano en rango manhattan (visión total)
    ...
    // Calcular proyectil en línea recta hacia el objetivo
    ...
    if(target.hp<=0){
      ...
      if(players.length){
        ...
        if(drop) applyDrop(sv,owner,drop,log);
...
function startSurvival(code,room){
  ...
  // 4 torretas en inventario de cada jugador (no colocadas aún)
  ...
  sv.log=['🎮 ¡Comenzó! 3 acciones/turno · Barreras ∞ · 4 torretas en inventario.'];
