...
function callClaude(prompt, maxTokens) {
  return new Promise((resolve) => {
    ...
    const req = https.request(options,(res)=>{let data='';res.on('data',c=>data+=c);res.on('end',()=>{try{resolve(JSON.parse(data).content.map(b=>b.text||'').join('').replace(/```json|```/g,'').trim());}catch{resolve(null);}});});
...
{"answers":["r1","r2","r3","r4","r5","r6","r7","r8","r9","r10"]}`;

  const t=await callClaude(prompt,2800);
  try{
    const clean=t.replace(/```json|```/g,'').trim();
    const p=JSON.parse(clean);
    if(Array.isArray(p.answers)&&p.answers.length>=count) return p.answers.slice(0,count);
    throw 0;
  }
  catch{
