...
function spawnMobs(sv, count) {
  ...
  const hpScale=Math.floor(turn/30); // escala lento: +1 HP cada 30 turnos
...
function startSurvival(code,room){
  ...
  // 4 torretas iniciales en esquinas internas
  const torretaPositions=[[4,4],[GRID-5,4],[4,GRID-5],[GRID-5,GRID-5]];
  torretaPositions.forEach((pos,i)=>{
    const owner=room.players[0]?.id||'init';
    sv.torretas.push({
      id:`t_init_${i}`,x:pos[0],y:pos[1],
      hp:10,maxHp:10,damage:1,range:5,active:true,ownerId:owner
    });
  });

  spawnMobs(sv,3);sv.turn=1;sv.phase='player_turn';
  sv.log=['🎮 ¡Comenzó! 3 acciones/turno · Barreras infinitas · 4 torretas activas.'];
