...
function genDrop(mob) {
  if(mob.type==='boss') return 'upgrade_point';
...
function applyDrop(sv, player, drop, log) {
...
function applyUpgrade(sv, player, upgradeId) {
  switch(upgradeId){
    ...
    case 'torreta': player.pendingTorreta=true; break;
    ...
    case 'upgrade_torreta':
      const t=sv.torretas.find(t=>t.ownerId===player.id);
      if(t){t.damage++;t.range++;t.hp=t.maxHp;} break;
  }
}
...
function spawnMobs(sv, count) {
  ...
  const hpScale=Math.floor(turn/10);
  // Escalar también con mejoras de jugadores
  const avgDmg=Object.values(sv.svPlayers).reduce((s,p)=>s+(p.damage||1),0)/Math.max(1,Object.values(sv.svPlayers).length);
  const dmgScale=Math.max(1,Math.floor(avgDmg*0.5));
  ...
  for(let i=0;i<count;i++){
    ...
    if(turn>40&&roll<0.1){type='elite';hp=5+hpScale+dmgScale;dmg=3;}
    else if(turn>20&&roll<0.2){type='tank';hp=3+hpScale+Math.floor(dmgScale/2);dmg=2;}
    else{hp=1+Math.floor(hpScale/2);}
    maxHp=hp;
...
function moveMobs(sv) {
  ...
  const log=[];
  const turn=sv.turn||1;
  // 0-10: random, 11-30: básico, 31-50: semi, 50+: full
  const aiLevel=turn<=10?0:turn<=30?1:turn<=50?2:3;

  sv.mobs=sv.mobs.map(mob=>{
    if(mob.stunned>0){mob.stunned--;return mob;}
    if(!players.length) return mob;
    const target=players.reduce((b,p)=>manhattan(mob,p)<manhattan(mob,b)?p:b,players[0]);
    ...
    for(const c of cands){
      ...
      if(barrierSet.has(`${c.x},${c.y}`)){
        ...
        if(bi>=0){sv.barriers[bi].hp--;if(sv.barriers[bi].hp<=0){sv.barriers.splice(bi,1);log.push('🧱 Barrera destruida');}}
        return mob;
      }
      if(torretaSet.has(`${c.x},${c.y}`)){
        const ti=sv.torretas.findIndex(t=>t.x===c.x&&t.y===c.y);
        if(ti>=0){sv.torretas[ti].hp--;if(sv.torretas[ti].hp<=0){sv.torretas.splice(ti,1);log.push('💥 Torreta destruida!');}}
        return mob;
      }
...
function torretasTurn(sv) {
  ...
  sv.torretas.forEach(t=>{
    ...
    if(target.hp<=0){
      const players=Object.values(sv.svPlayers).filter(p=>p.alive);
      if(players.length){
        const closest=players.reduce((b,p)=>manhattan(t,p)<manhattan(t,b)?p:b,players[0]);
        sv.mobsDefeated++;
        closest.killCount=(closest.killCount||0)+1;
        const drop=genDrop(target);
        if(drop) applyDrop(sv,closest,drop,log);
        if(closest.killCount%10===0){
          closest.upgradePoints=(closest.upgradePoints||0)+1;
          log.push(`⭐ ${closest.name}: ${closest.killCount} kills — ¡punto de mejora!`);
        }
      }
      sv.mobs=sv.mobs.filter(m=>m.id!==target.id);
      log.push(`🤖 Torreta eliminó ${target.type}!`);
    } else {
      log.push(`🤖 Torreta dañó ${target.type} (${target.hp}HP)`);
    }
  });
  return log;
}
...
io.on('connection', (socket) => {
  ...
  socket.on('sv_action', ({action,targetX,targetY,upgradeId}) => {
    ...
    if(action==='upgrade'){
      ...
      if(upgradeId==='torreta'){socket.emit('sv_place_torreta');sv.log.push(logMsg);broadcastSV(code,room);return;}
      ...
      sv.torretas.push({id:`t${Date.now()}`,x:targetX,y:targetY,hp:4,maxHp:4,damage:1,range:4,active:true,ownerId:socket.id});
      p.pendingTorreta=false;
      logMsg=`🤖 ${p.name} construyó torreta`;
    } else if(action==='move'){
      ...
      if(Math.abs(targetX-p.x)>MOVE_RANGE||Math.abs(targetY-p.y)>MOVE_RANGE){socket.emit('sv_error','Solo casillas adyacentes');return;}
      ...
      while(inBounds(cx,cy)&&steps<playerRange){
        ...
        if(mi>=0){
          ...
          if(sv.mobs[mi].hp<=0){
            const drop=genDrop(sv.mobs[mi]);
            if(drop) applyDrop(sv,p,drop,sv.log);
            sv.mobs.splice(mi,1);
            sv.mobsDefeated++;
            p.killCount=(p.killCount||0)+1;
            logMsg=`🎯 ${p.name} eliminó enemigo! (${p.killCount} kills)`;
            if(p.killCount%10===0){
      ...
      sv.barriers.push({x:targetX,y:targetY,hp:3});p.barriers--;
...
function startSurvival(code,room){
  ...
  room.players.forEach((pl,i)=>{
    sv.svPlayers[pl.id]={
      ...
      barriers:3,shields:2,heals:3,shielded:0,
      acted:false,actionsUsed:0,maxActions:1,
      damage:1,shootRange:BASE_SHOOT_RANGE,
      killCount:0,upgradePoints:0,
      healPower:2,shieldDur:3,maxBarriers:5,pendingTorreta:false
    };
  });
  spawnMobs(sv,3);sv.turn=1;sv.phase='player_turn';
  sv.log=['🎮 ¡Comenzó! Disparo en línea recta — alcance total del mapa.'];
...
function doMobTurn(code,room){
  ...
  const torretaLog=torretasTurn(sv);
  ...
  if(sv.turn>=50&&!sv.aiActive){
    sv.aiActive=true;
    sv.log.push('🧠 ¡IA COMPLETA activada — flanqueo inteligente!');
    io.to(code).emit('sv_ai_on');
  }

  if(sv.spawnCounter>=3){
    sv.spawnCounter=0;
    const wave=Math.min(1+Math.floor(sv.turn/5),8);
    spawnMobs(sv,wave);
