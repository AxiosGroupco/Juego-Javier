...
io.on('connection', (socket) => {
  ...
  socket.on('dnd_buy_item', async ({code, itemId}) => {
    ...
    // Sync to DB
    if(session.dbCharacterId) {
      try { await db.updateCharacter(session.dbCharacterId, {items:player.items, hp:player.hp, stamina:player.stamina||0, mana:player.mana||0, int_stat:player.int}); } catch(e) {}
    }
  });
  ...
  socket.on('dnd_join_party', async ({code, characterId}) => {
    ...
    try {
      ...
      socket.join(code); socket.roomCode=code;
      io.to(code).emit('dnd_party_updated', {party: room.dndParty.map(p=>({socketId:p.socketId,characterName:p.characterName,characterClass:p.characterClass}))});
    } catch(e) { socket.emit('dnd_error','Error uniéndose'); }
  });

  socket.on('dnd_start_party', async ({code}) => {
    const authUser = socketUsers[socket.id];
    const room = rooms[code];
    if(!room||room.game!=='dnd'||room.phase!=='lobby') return socket.emit('dnd_error','Sala no disponible');
