...
async function generateScene(session, actionTaken, actingPlayer) {
  ...
  const partyLevels = session.players.map(p=>
    `${p.characterName}: Nivel ${p.level||1} (XP: ${p.xp||0}/${getXpToNextLevel(p.level||1)})`
  ).join(', ');
...
Genera la respuesta del DM. JSON ESTRICTO:
{
  ...
  "xpGrants": [
    {"reason": "enemy_defeated|boss_killed|puzzle_solved|clever_action|near_death|chapter_complete", "amount": 15, "toAll": true}
  ]
}`;
...
// XP para subir de nivel N → N+1

const XP_REWARDS = {
  chapter_complete:  80,   // completar un capítulo
  boss_killed:       60,   // matar un jefe
  puzzle_solved:     40,   // resolver un puzzle
  enemy_defeated:    15,   // derrotar un enemigo normal
  clever_action:     25,   // acción especialmente creativa (el DM decide)
  near_death:        20,   // sobrevivir con HP muy bajo
  sacrifice:         35,   // sacrificarse por el grupo
};

function getXpToNextLevel(level) {
  return XP_TABLE[Math.min(level, XP_TABLE.length-1)] || 4000;
}

function grantXP(session, playerId, amount, reason, io, code) {
  const player = session.players.find(p=>p.id===playerId);
  if(!player || !player.alive) return;
  player.xp = (player.xp||0) + amount;
  session.log = session.log || [];
  session.log.push(`✨ ${player.characterName} +${amount} XP (${reason})`);
  checkLevelUp(player, session, io, code);
}

function grantXPAll(session, amount, reason, io, code) {
  session.players.filter(p=>p.alive).forEach(p=>{
    p.xp = (p.xp||0) + amount;
    checkLevelUp(p, session, io, code);
  });
  session.log = session.log || [];
  session.log.push(`✨ Todo el grupo +${amount} XP (${reason})`);
}

function checkLevelUp(player, session, io, code) {
  const needed = getXpToNextLevel(player.level||1);
  if(player.xp >= needed && (player.level||1) < 10) {
    player.level = (player.level||1) + 1;
    player.xp = player.xp - needed;
    player.skillPtsAvailable = (player.skillPtsAvailable||0) + 2;
    player.statPtsAvailable  = (player.statPtsAvailable||0)  + 1;
    player.maxHp += 2; player.hp = Math.min(player.hp+2, player.maxHp);
    // Aumentar recurso también
    if(player.maxStamina>0) player.maxStamina+=2;
    if(player.maxMana>0)    player.maxMana+=2;
    player.pendingLevelUp = true;
    session.log.push(`🎉 ¡${player.characterName} subió al nivel ${player.level}! +2 pts habilidad, +1 stat`);
    if(io && code) {
      io.to(code).emit('dnd_level_up', {
        playerId: player.id,
        playerName: player.characterName,
        newLevel: player.level,
        skillPtsAvailable: player.skillPtsAvailable,
        statPtsAvailable: player.statPtsAvailable
      });
    }
    // Puede subir más de un nivel si tiene XP suficiente
    checkLevelUp(player, session, io, code);
  }
}
