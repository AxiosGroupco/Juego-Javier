...
// ── OPTIMIZACIÓN: intro + misiones en UNA sola llamada ─────────
async function generateUniverseEntry(universe,character,history,legends){
  // Contexto mínimo del universo (evitar enviar lore completo)
  const lore=universe.lore||{};
  const univCtx=`${lore.age||''} | Conflicto: ${(lore.currentConflict||'').slice(0,120)} | Facciones: ${(lore.factions||[]).slice(0,2).join(', ')}`;
  const histCtx=history.slice(0,5).map(h=>`${h.title}`).join('; ')||'Sin eventos previos';
  // Solo 2 leyendas recortadas
  const legsCtx=legends.slice(0,2).map(l=>`${l.title}: ${l.description.slice(0,80)}`).join(' | ')||'';
  const charLevel=character.level||1;
  const prompt=`Universo: "${universe.name}" (${universe.atmosphere||'epic'})
Contexto: ${univCtx}
Historia reciente: ${histCtx}
Leyendas: ${legsCtx||'Ninguna aún'}
Personaje: ${character.name} (${character.class} Nv${charLevel}) — "${(character.backstory||'Sin historia').slice(0,120)}"

Responde SOLO JSON:
{"introNarration":"narración bienvenida 3 párrafos conectando backstory con universo","missions":[{"title":"nombre","summary":"1 oración","hook":"conexión con backstory del personaje (15 palabras max)","minLevel":1,"maxLevel":3,"difficulty":"easy|normal|hard|legendary","location":"lugar","antagonist":"enemigo"},{"title":"","summary":"","hook":"","minLevel":1,"maxLevel":4,"difficulty":"normal","location":"","antagonist":""},{"title":"","summary":"","hook":"","minLevel":${charLevel+1},"maxLevel":${charLevel+3},"difficulty":"hard","location":"","antagonist":""},{"title":"","summary":"","hook":"","minLevel":${charLevel+2},"maxLevel":${charLevel+5},"difficulty":"legendary","location":"","antagonist":""}]}`;
  const result=await callClaude([{role:'user',content:prompt}],1600,DM_SYSTEM);
  try{
    const p=JSON.parse(result);
    return{introNarration:p.introNarration||'',missions:Array.isArray(p.missions)?p.missions:[]};
  }catch{return{introNarration:'',missions:[]};}
}
// Alias para compatibilidad
async function generateUniverseIntro(universe,character,legends){
  const r=await generateUniverseEntry(universe,character,[],legends);
  return r.introNarration;
}
async function generateMissions(universe,character,history,count=4){
  const r=await generateUniverseEntry(universe,character,history,[]);
  return r.missions.slice(0,count);
}
